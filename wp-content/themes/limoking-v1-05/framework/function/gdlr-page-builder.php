<?php
	...
	*/	
	...
	if( !class_exists('limoking_page_builder') ){
		
		class limoking_page_builder{
			...
			function __construct($setting = array(), $option = array()){
				...
				$default_setting = array(
					...
					'section' => array(
						'above-sidebar' => array( 
							'title' => esc_html__('Above Sidebar Section', 'limoking'),
							'class' => 'above-sidebar-section',
						),
						'content-with-sidebar' => array( 
							'title' => esc_html__('Content ( With Sidebar ) Section', 'limoking'),
							'class' => 'with-sidebar-section',
						),
						'below-sidebar' => array( 
							'title' => esc_html__('Below Sidebar Section', 'limoking'),
				...
				add_action('wp_ajax_limoking_print_tinymce_editor', array(&$this, 'limoking_print_tinymce_editor'));					
			}		
			...
			function load_admin_script(){
				...
				wp_enqueue_style('limoking-size-controls', get_template_directory_uri() . '/framework/stylesheet/gdlr-size-controls.css');	
				...
				wp_enqueue_style('limoking-page-builder', get_template_directory_uri() . '/framework/stylesheet/gdlr-page-builder.css');					
				...
				wp_enqueue_script('limoking-edit-box-tab', get_template_directory_uri() . '/framework/javascript/gdlr-edit-box-tab.js');
				wp_enqueue_script('limoking-slider-selection', get_template_directory_uri() . '/framework/javascript/gdlr-slider-selection.js');
				wp_enqueue_script('limoking-page-builder', get_template_directory_uri() . '/framework/javascript/gdlr-page-builder.js');
				
				wp_localize_script( 'limoking-edit-box', 'GDLR', array('ajax_url'=>AJAX_URL) );
			}	
			...
			function create_page_builder_elements(){		
				echo '<div class="limoking-page-builder" id="limoking-page-builder">';
				...
				echo '</div>'; // limoking-page-builder
			}
			...
			function print_page_builder_creator(){
				...
				echo '<h4 class="page-builder-head add-content">' . esc_html__('Add Content Item', 'limoking') . '</h4>';
				echo '</div>';
				...
				foreach( $this->option as $add_item_slug => $add_item_wrapper ){
					...
					foreach( $add_item_wrapper['options'] as $item_slug => $item_wrapper ){
						if( !empty($item_wrapper) ){
							...
							echo limoking_escape_content($item_wrapper['title']) . '</option>';
						}
					}
					echo '</select>';
			...
			function print_page_builder_default_item(){
				$page_builder_html = new limoking_page_builder_html();
			...
			function print_page_builder_content(){
				...
				$page_builder_html = new limoking_page_builder_html( $this->merge_page_builder_items() );
				
				// head section
				echo '<div class="page-builder-head-wrapper">';
				echo '<h4 class="page-builder-head page-builder">' . esc_html__('Page Builder Section', 'limoking') . '</h4>';
				
				echo '<div class="command-button-wrapper">';
				echo '<input class="undo-button" type="button" value="' . esc_html__('Undo', 'limoking') . '" />';
				echo '<input class="redo-button" type="button" value="' . esc_html__('Redo', 'limoking') . '" />';
				echo '</div>';	
				...
				foreach( $this->setting['section'] as $section_slug => $section ){
					$value = limoking_decode_preventslashes(get_post_meta($post->ID, $section_slug, true));
					...
					echo '<div class="limoking-sortable-wrapper" data-type="' . $section['class'] . '" >';
					echo '<div class="page-builder-item-area limoking-sortable clear-fix row ';
					echo (!empty($array_value))? '': 'blank';
					echo '" >';
					$page_builder_html->print_page_builder( $array_value );	
					echo '</div>';
					echo '</div>'; // limoking-sortable-wrapper
					
					echo '<textarea class="limoking-input-hidden" name="' . $section_slug . '" >' . esc_textarea($value) . '</textarea>';
			...
			function limoking_print_tinymce_editor(){
				wp_editor( limoking_stripslashes($_POST['content']), 
			...
			function save_page_builder( $post_id ){
				foreach( $this->setting['section'] as $section_slug => $section ){
					if( isset($_POST[$section_slug]) ){
						update_post_meta($post_id, $section_slug, limoking_preventslashes($_POST[$section_slug]));
					}
				}
			}
