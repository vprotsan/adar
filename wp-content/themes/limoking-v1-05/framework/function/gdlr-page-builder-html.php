<?php
	...
	*/	
	...
	if( !class_exists('limoking_page_builder_html') ){
		
		class limoking_page_builder_html{
			...
			function print_draggable_wrapper($item_atts = array()){
				echo '<div class="limoking-draggable limoking-draggable-wrapper ';
				echo (!empty($item_atts['size']))? $this->gdl_size[$item_atts['size']] : $this->gdl_size['1/1'];
				echo '" data-type="' . $item_atts['slug'] . '">';
				echo '<div class="limoking-item">';
				
				echo '<div class="limoking-draggable-head">';
				if(!empty($item_atts['size'])){ 
					echo '<div class="limoking-size-wrapper">';
					echo '<div class="limoking-size-control">';
					echo '<div class="limoking-increase-size"></div>';
					echo '<div class="limoking-decrease-size"></div>';
					echo '</div>';
					
					echo '<div class="limoking-size-text">' . $item_atts['size'] . '</div>';
					echo '</div>'; // limoking-size-wrapper 
				}
				
				$item_title = empty($item_atts['item-builder-title'])? $item_atts['title']: $item_atts['item-builder-title'];
				echo '<div class="limoking-draggable-text">';
				echo '<input type="text" class="limoking-draggable-text-input" value="' . $item_title . '" />';
				echo '</div>';
				
				echo (!empty($item_atts['options']))? '<div class="limoking-edit-item"></div>': ''; 

				echo '<div class="limoking-delete-item"></div>';
				echo '<div class="clear"></div>';
				echo '</div>';
				
				echo '<div class="limoking-inner-sortable limoking-sortable clear-fix ' . $item_atts['slug'] . '-sortable">';
				if( !empty($item_atts['inner-item']) ){
					$this->print_page_builder($item_atts['inner-item']);
				}
				echo '</div>';
				
				echo '</div>'; // limoking-item
				
				if(!empty($item_atts['options'])){
					$this->print_item_option($item_atts['options']);
				}
				echo '</div>'; // limoking-draggable-wrapper			
			}
			
			// print the draggable item type
			function print_draggable_item($item_atts = array()){
				echo '<div class="limoking-draggable limoking-draggable-item ' . $this->gdl_size['1/1'] . '" ';
				echo 'data-type="' . $item_atts['slug'] . '" ';
				echo '>';
				echo '<div class="limoking-item">';
				...
				if(!empty($item_atts['options'])){ 
					echo '<div class="limoking-edit-item"></div>'; 
				}
				echo '<div class="limoking-delete-item"></div>';
				echo '</div>';			
				...
				echo '</div>'; //limoking-draggable-item			
			}
			
			// print default option
			function print_item_option($options){
				echo '<div class="limoking-item-option">';
				...
				foreach($options as $option_slug => $option){
					...
					foreach( $option as $att_slug => $att ){
						if( !is_array( $att ) ){
							echo 'data-' . $att_slug . '="' . esc_attr(limoking_esc_quot($att)) . '" ';
						}
					}					
					echo '>';
					...
					if( !empty($option['type']) && $option['type'] == 'upload' ){
						if( !empty($option['value']) && is_numeric($option['value']) ){ 
							...
							echo limoking_escape_content($thumbnail[0]);
						}else if( !empty($option['value']) ){
							echo limoking_escape_content($option['value']);
						}
					}
					echo '</div>';
				}
