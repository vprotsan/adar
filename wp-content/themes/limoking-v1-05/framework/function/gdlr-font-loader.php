<?php
	...
	*/	
	...
	if( !class_exists('limoking_font_loader') ){	
		class limoking_font_loader{
			...
			function __construct( $custom_font ){
				...
				add_filter('limoking_enqueue_scripts', array(&$this, 'include_google_font'));
				add_filter('limoking_style_custom_end', array(&$this, 'add_custom_style'), 1, 2);
				
				add_action('limoking_print_all_font_list', array(&$this, 'print_all_font_list'));
			}
			...
			function set_google_font_list(){
				$current_page =  wp_nonce_url(admin_url('themes.php?page=limoking'),'limoking-theme-option');
				$google_font_file = apply_filters('limoking_google_font_file', get_template_directory() . '/framework/function/gdlr-google-font.txt');
				$google_fonts = json_decode(limoking_read_filesystem($current_page, $google_font_file), true);
			...
			function print_all_font_list( $selected_font ){
				// custom font section
				echo '<option disabled >------- ' . esc_html__('Custom Font' ,'limoking') . ' -------</option>';
				foreach( $this->custom_font_list as $font_family => $font ){
					if( !empty($font_family) ){
						...
						echo limoking_escape_content($font_family) . '</option>';
					}
				}					

				// safe font section
				echo '<option disabled >------ ' . esc_html__('Web Safe Font' ,'limoking') . ' ------</option>';
				foreach( $this->safe_font_list as $font ){
					echo '<option data-type="web-safe-font" ';
					echo ($font == $selected_font)? 'selected >': '>';
					echo limoking_escape_content($font) . '</option>';
				}
				
				// google font section
				echo '<option disabled >------- ' . esc_html__('Google Font' ,'limoking') . ' -------</option>';
			...
			function get_google_font_url( $font_family ){
				if( !empty($font_family) && !empty($this->google_font_list[$font_family]) ){
					...
					$temp_font_name .= apply_filters('limoking_google_font_weight', implode(',', $google_font['variants'])) . '&subset='; 
					$temp_font_name .= apply_filters('limoking_google_font_subset', implode(',', $google_font['subsets'])); 
