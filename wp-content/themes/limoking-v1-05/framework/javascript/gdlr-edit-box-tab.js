(function($){
	...
	function gdlrCreateTabItem( options ){
		...
		// bind the color picker button
		tab_item.find('.wp-color-picker').wpColorPicker({
			change: function(){
				gdlrSaveTabAction(settings.textarea);
			}
		});		
		...
		tab_item.find('.limoking-upload-wrapper .gdl-text-input').each(function(){
			if( $(this).val() ){
			...
			$(this).change(function(){
				...
				if( $(this).val() == '' ){ 
					$(this).siblings('.limoking-upload-img-sample').addClass('blank').trigger('change'); 
				}else{
					$(this).siblings('.limoking-upload-img-sample').attr('src', $(this).val()).removeClass('blank').trigger('change');
				}
			});
		});
		tab_item.find('.limoking-upload-box-button').click(function(){
			...
			var custom_uploader = wp.media({
				...
				upload_button.siblings('.limoking-upload-box-hidden').val(attachment.id).trigger('change');
			}).open();			
		});		
		...
		$(this).gdlrAddMoreTabs({
			default_item	: 	'<div class="edit-box-input-wrapper">\
								...
								<div class="edit-box-input-wrapper">\
									...
									<div class="limoking-combobox-wrapper">\
			...
			textarea		: 	$('<textarea class="limoking-input-hidden" name="' + option.attr('data-name') + '">' + value + '</textarea>')
		});
	}
	...
	$.fn.gdlrEditBoxTab = function( option ){	
		...
		$(this).gdlrAddMoreTabs({
			...
			default_title	: 	option.attr('data-default-title'),
			textarea		: 	$('<textarea class="limoking-input-hidden" name="' + option.attr('data-name') + '">' + limoking_esc_attr(value) + '</textarea>')
		});
	}
	...
	$.fn.gdlrEditBoxIconWithList = function( option ){	
		...
		$(this).gdlrAddMoreTabs({
			default_item	: 	'<div class="edit-box-input-wrapper">\
									<div class="input-box-title">Image (Icon will be omitteed)</div>\
									<div class="edit-box-input">\
									<div class="limoking-upload-wrapper">\
										<img class="limoking-upload-img-sample blank" src="">\
										<div class="clear"></div>\
										<input type="text" class="gdl-text-input" data-name="gdl-tab-image-url" >\
										<input type="text" class="limoking-upload-box-hidden" data-name="gdl-tab-image">\
										<input type="button" class="limoking-upload-box-button gdl-button" value="Upload"></div>\
										<div class="clear"></div>\
									</div>\
								</div>\
								<div class="edit-box-input-wrapper">\
	...
	$.fn.gdlrEditBoxAuthor = function( option ){	
		...
		var default_input = '<div class="edit-box-input-wrapper">\
								...
								<div class="limoking-upload-wrapper">\
									...
									<input type="text" class="limoking-upload-box-hidden" data-name="gdl-tab-author-image">\
									<input type="button" class="limoking-upload-box-button gdl-button" value="Upload"></div>\
									<div class="clear"></div>\
								</div>\
							</div>\
	...
	// get input list
	function gdlrGetInputHtml(slug, option_text){
		var options = option_text.split(':');
		var label = (options[0])? options[0]: '';
		var type = (options[1])? options[1]: '';
		var default_val = (options[2])? options[2]: '';
		
		switch (type){
			case 'text':
				return	'<div class="edit-box-input-wrapper">\
							<div class="input-box-title">' + label + '</div>\
							<input type="text" class="gdl-text-input" data-name="' + slug + '" value="' + default_val + '" />\
						</div>';
				break;
			case 'textarea':
				return	'<div class="edit-box-input-wrapper">\
							<div class="input-box-title">' + label + '</div>\
							<textarea data-name="' + slug + '" >' + default_val + '</textarea>\
						</div>';
				break;
			case 'colorpicker':
				return	'<div class="edit-box-input-wrapper">\
							<div class="input-box-title">' + label + '</div>\
							<input type="text" class="wp-color-picker" data-name="' + slug + '" value="' + default_val + '" data-default-color="' + default_val + '" />\
						</div>';
			case 'upload':
				return 	'<div class="edit-box-input-wrapper">\
							<div class="input-box-title">' + label + '</div>\
							...
							<div class="limoking-upload-wrapper">\
								...
								<div class="clear"></div>\
								<input type="text" class="gdl-text-input" data-name="gdl-tab-author-image-url" >\
								<input type="text" class="limoking-upload-box-hidden" data-name="' + slug + '">\
								<input type="button" class="limoking-upload-box-button gdl-button" value="Upload"></div>\
								<div class="clear"></div>\
							</div>\
						</div>';
	...
	// for chart item
	$.fn.gdlrEditBoxChart = function( option ){
		var value = option.attr('data-value');
		if (typeof value == 'undefined' && option.attr('data-default')) {
			value = option.attr('data-default');
		}else if(typeof value == 'undefined'){
		...
		for (var slug in options) {
			default_input += gdlrGetInputHtml(slug, options[slug]);
		}
