(function($){
	...
	$(document).ready(function(){
		...
		$('#publish, #preview-action a, #save-post, #post-preview').click(function(){
			...
			var page_option = $('.limoking-page-option-wrapper');
			...
			page_option.each(function(){
				...
				$(this).children('textarea.limoking-input-hidden').val(JSON.stringify(page_option));
			});

		});
		
		// load page builder meta
		$('#limoking-load-demo-wrapper').each(function(){
			var post_id = $(this).attr('data-id');
			var ajax_url = $(this).attr('data-ajax');
			var action = $(this).attr('data-action');	
			
			$(this).children('input[type="button"]').click(function(){
				var button_slug = $(this).siblings('select').val();
