(function($){
	...
	function gdlrCreateSliderSlide(textarea){
		...
		for (var i=0; i<slide_order.length; i++){ 
			var slide = $('<div class="limoking-slide-wrapper" data-id="' + slide_order[i] + '" ></div>');
			...
			var remove = $('<div class="limoking-delete-slide"></div>');
			remove.click(function(){
				var remove_button = $(this);
				$('body').limoking_confirm({
					success: function(){
						remove_button.parent('.limoking-slide-wrapper').slideUp(function(){
							$(this).remove();
							gdlrUpdateSliderOrder(textarea);
						});					
					}
	...
	function gdlrCreateSliderOption(textarea, image_id){
		...
		if(overlay){ $('body').addClass('limoking-disable-scroll'); }
	...
	function gdlrCloseSliderOption(editbox, textarea, image_id, overlay){
		...
		if(overlay){ $('body').removeClass('limoking-disable-scroll'); }	
	}
