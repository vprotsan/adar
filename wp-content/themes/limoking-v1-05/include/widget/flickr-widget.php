...
add_action( 'widgets_init', 'limoking_flickr_widget' );
if( !function_exists('limoking_flickr_widget') ){
	function limoking_flickr_widget() {
...
if( !class_exists('Goodlayers_Flickr_Widget') ){
	class Goodlayers_Flickr_Widget extends WP_Widget{
		...
		function __construct() {
			parent::__construct(
				'gdlr-flickr-widget', 
				esc_html__('Goodlayers Flickr Widget','limoking'), 
				array('description' => esc_html__('A widget that show image from flickr', 'limoking')));  
		}
		...
		function widget( $args, $instance ) {
			...
			if(!empty($id)){ 
				...
				?>
					<div class="limoking-flickr-widget">
					<script type="text/javascript" src="http://www.flickr.com/badge_code_v2.gne<?php echo limoking_escape_content($flickr); ?>"></script>
					<div class="clear"></div>
					</div>
				<?php
			}
		...
		function form( $instance ) {
			...
			<!-- ID --> 
			<p>
				<label for="<?php echo $this->get_field_id('id'); ?>"><?php esc_html_e('Flickr ID :', 'limoking'); ?></label>
				<input class="widefat" id="<?php echo $this->get_field_id('id'); ?>" name="<?php echo $this->get_field_name('id'); ?>" type="text" value="<?php echo limoking_escape_content($id); ?>" />
			</p>			
			...
			<!-- Order By -->
			<p>
				<label for="<?php echo $this->get_field_id('orderby'); ?>"><?php esc_html_e('Order By :', 'limoking'); ?></label>		
				<select class="widefat" name="<?php echo $this->get_field_name('orderby'); ?>" id="<?php echo $this->get_field_id('orderby'); ?>">
					<option value="latest" <?php if(empty($orderby) || $orderby == 'latest') echo ' selected '; ?>><?php esc_html_e('Latest', 'limoking') ?></option>
					<option value="random" <?php if($orderby == 'random') echo ' selected '; ?>><?php esc_html_e('Random', 'limoking') ?></option>				
				</select> 
			</p>
