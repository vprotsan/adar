...
add_action( 'widgets_init', 'limoking_popular_post_widget' );
if( !function_exists('limoking_popular_post_widget') ){
	function limoking_popular_post_widget() {
	...
	class Goodlayers_Popular_Post extends WP_Widget{
		...
		function __construct() {
			parent::__construct(
				...
				esc_html__('Goodlayers Popular Post Widget','limoking'), 
				array('description' => esc_html__('A widget that show popular posts ( by comment )', 'limoking')));  
		}
		...
		function widget( $args, $instance ) {
			...
			if($query->have_posts()){
				echo '<div class="limoking-recent-post-widget">';
				while($query->have_posts()){ $query->the_post();
					echo '<div class="recent-post-widget">';
					$thumbnail = limoking_get_image(get_post_thumbnail_id(), 'thumbnail');
					if( !empty($thumbnail) ){
					...
					echo '<i class="fa ' . limoking_fa_class('icon-comment') . '"></i>';
					comments_popup_link( esc_html__('0 Comment','limoking'),
						esc_html__('1 Comment','limoking'),
						esc_html__('% Comments','limoking'), '',
						esc_html__('Comments are Off','limoking') );					
					echo '</div>'; // blog-info
		...
		function form( $instance ) {
			...
			<p>
				...
				<?php 	
				$category_list = limoking_get_term_list('category'); 
