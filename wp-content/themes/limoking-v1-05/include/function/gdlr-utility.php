<?php
	...
	*/
	...
	if( !function_exists('limoking_get_logo') ){
		function limoking_get_logo(){
			global $theme_option;
?>
<!-- logo -->
<div class="limoking-logo">
	...
	<?php
		// mobile navigation
		if( class_exists('limoking_dlmenu_walker') && has_nav_menu('main_menu') &&
			( empty($theme_option['enable-responsive-mode']) || $theme_option['enable-responsive-mode'] == 'enable' ) ){
			echo '<div class="limoking-responsive-navigation dl-menuwrapper" id="limoking-responsive-navigation" >';
			echo '<button class="dl-trigger">Open Menu</button>';
			wp_nav_menu( array(
				'theme_location'=>'main_menu',
				'container'=> '',
				'menu_class'=> 'dl-menu limoking-main-mobile-menu',
				'walker'=> new limoking_dlmenu_walker()
			) );
			echo '</div>';
		}
	?>
			...
			if( !empty($theme_option['skin-settings']) ){
				...
				if( !empty($skins) ){
					foreach( $skins as $skin ){
						$skin_list[limoking_string_to_class($skin['skin-title'])] = $skin['skin-title'];
					}
				}
			}
			return $skin_list;
