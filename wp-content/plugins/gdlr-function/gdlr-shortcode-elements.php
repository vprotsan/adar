<?php
	...
	function gdlr_accordion_shortcode( $atts, $content = null ){
		...
		if( function_exists('limoking_get_accordion_item') ){
			return '<div class="limoking-shortcode-wrapper">' . limoking_get_accordion_item($settings) . '</div>';
		}
		return '';
	}	
	...
	function gdlr_toggle_box_shortcode( $atts, $content = null ){
		...
		if( function_exists('limoking_get_toggle_box_item') ){
			return '<div class="limoking-shortcode-wrapper">' . limoking_get_toggle_box_item($settings) . '</div>';
		}
		return '';
	}	
	...
	function gdlr_process_shortcode( $atts, $content = null ){
		...
		$ret  = '<div class="limoking-shortcode-wrapper">';
		$ret .= '<div class="limoking-process-wrapper limoking-' . $type . '-process">';
		
		$size = sizeof($gdlr_shortcode_tabs); $item_num = $size;
		foreach( $gdlr_shortcode_tabs as $tab ){
			$ret .= ($type == 'horizontal')? '<div class="' . limoking_get_column_class('1/' . $size) . '">': '';
			$ret .= '<div class="limoking-item limoking-process-tab" ' . $style . '>';
			if( !empty($tab['gdl-tab-icon']) ){
				$ret .= '<div class="limoking-process-icon" >';
				$ret .= '<i class="fa ' . limoking_fa_class($tab['gdl-tab-icon']) . '" ></i>';
				$ret .= '</div>';
			}
			$ret .= '<div class="limoking-process-tab-content" >';
			$ret .= '<h3 class="limoking-process-title" >' . $tab['gdl-tab-title'] . '</h3>';
			$ret .= '<div class="limoking-process-caption" >' . $tab['gdl-tab-content'] . '</div>';
			$ret .= '</div>'; // limoking-process-tab-content
			
			if( $item_num > 1 ){
				$ret .= '<div class="process-line">';
				$ret .= '<div class="process-line-divider"></div>';
				$ret .= ($type == 'horizontal')? '<div class="fa ' . limoking_fa_class('icon-chevron-right') . '" ></div>': '<div class="fa ' . limoking_fa_class('icon-chevron-down') . '" ></div>';
				$ret .= '</div>';
			}
			
			$ret .= '</div>'; // limoking-process-tab-wrapper
	...
	function gdlr_price_table_shortcode( $atts, $content = null ){
		...
		if( function_exists('limoking_get_price_table_item') ){
			return '<div class="limoking-shortcode-wrapper">' . limoking_get_price_table_item($settings) . '</div>';
		}
		return '';
	}		
	...
	function gdlr_tabs_shortcode( $atts, $content = null ){
		...
		if( function_exists('limoking_get_tab_item') ){
			return '<div class="limoking-shortcode-wrapper">' . limoking_get_tab_item($settings) . '</div>';
		}
		return '';
	}		
	...
	function gdlr_tab_shortcode( $atts, $content = null ){
		...
		array_push($gdlr_shortcode_tabs, array(
			'gdl-tab-icon'=>limoking_fa_class($icon), 
	...
	function gdlr_button_shortcode( $atts, $content = null ){
		...
		return '<a class="limoking-button ' . $button_class . '" href="' . $href . '" target="' . $target . '" ' . $style . ' >' . do_shortcode($content) . '</a>';
	}	
	...
	function gdlr_heading_shortcode( $atts, $content = null ){
		...
		$ret  = '<' . $tag . ' class="limoking-heading-shortcode ' . $class . '" ' . $style . '>';
		$ret .= empty($icon)? '': '<i class="fa ' . limoking_fa_class($icon) . '" ></i>'; 
		$ret .= $content;
		$ret .= '</' . $tag . '>';
		return $ret;
	}	
	...
	function gdlr_divider_shortcode( $atts ){
		...
		if( function_exists('limoking_get_divider_item') ){
			return '<div class="limoking-shortcode-wrapper">' . limoking_get_divider_item($settings) . '</div>';
		}
		return '';
	}
	...
	function gdlr_circle_progress_shortcode( $atts, $content = null ){
		...
		$ret  = '<div class="limoking-chart limoking-ux" data-percent="' . $percent . '" data-size="' . $size . '" data-linewidth="' . $line_width . '" ';
	...
	function gdlr_stunning_text_shortcode( $atts, $content = null ){
		...
		$stunning_class .= (empty($button) || empty($button_link))? '': 'limoking-button-on ';
		...
		$ret  = '<div class="limoking-stunning-text-ux limoking-ux">';
		$ret .= '<div class="limoking-stunning-text-item ' . $stunning_class . '" ' . $stunning_style . ' >';
		if(!empty($title)){
		...
		if(!empty($button) && !empty($button_link)){
			...
			$ret .= '<a class="stunning-text-button limoking-button with-border" href="' . $button_link . '" ' . $stunning_style . '>';
			$ret .= $button;
			$ret .= '</a>';
		}
	...
	function gdlr_skill_bar_shortcode( $atts, $content = null ){
		...
		$settings['icon'] = limoking_fa_class($icon);
		$settings['content'] = $content;
	
		if( function_exists('limoking_get_skill_bar_item') ){
			return '<div class="limoking-shortcode-wrapper">' . limoking_get_skill_bar_item($settings) . '</div>';
		}
		return '';
	}	
	...
	function gdlr_row_shortcode( $atts, $content = null ){
		return '<div class="limoking-shortcode-wrapper limoking-row-shortcode">' . do_shortcode($content) . '<div class="clear"></div></div>';
	}	
	function gdlr_column_shortcode( $atts, $content = null ){
		...
		if( function_exists('limoking_get_column_class') ){
			$ret  = '<div class="' . limoking_get_column_class($size) . '">';
			$ret .= '<div class="limoking-item limoking-column-shortcode">';
	...
	function gdlr_frame_shortcode( $atts, $content = null ){
		...
		$ret  = '<div class="limoking-frame-shortcode limoking-align-' . $align . '">';
		$ret .= '<div class="limoking-frame frame-type-' . $type . '" ' . $style . '>';
		$ret .= do_shortcode($content);
		$ret .= '</div>';
		if( !empty($caption) ){
			$ret .= '<span class="limoking-frame-caption">' . $caption . '</span>';
		}
		$ret .= '</div>';
		return $ret;
	}	
	...
	function gdlr_image_link_shortcode( $atts ){
		...
		$ret  = '<div class="limoking-image-link-shortcode" ' . $style . ' >';
		$ret .= '<img src="' . $image_url . '" alt="' . $alt . '" />';
		$ret .= '<a href="' . $link_url . '" ' . $link_attr . ' target="' . $target . '" >';
		$ret .= '<span class="limoking-image-link-overlay">&nbsp;</span>';
		$ret .= '<span class="limoking-image-link-icon"><i class="fa ' . limoking_fa_class($link_icon) . '" ></i></span>';
		$ret .= '</a>';
		$ret .= '</div>';
		return $ret;
	}	
	...
	function gdlr_space_shortcode( $atts ){
		...
		$ret .= '<span class="limoking-space" style="margin-top: ' . $height . '; display: block;"></span>';
		return $ret;
	}	
	...
	function gdlr_quote_shortcode($atts, $content = null){
		...
		return '<blockquote class="limoking-align-' . $align . '" >' . do_shortcode($content) . '</blockquote>';
	}	
	...
	function gdlr_dropcap_shortcode($atts, $content = null){
		...
		return '<div class="limoking-dropcap limoking-type-' . $type . '" ' . $style . ' >' . $content . '</div>';
	}	
	
	// icon shortcode
	add_shortcode('gdlr_icon', 'gdlr_icon_shortcode');
	function gdlr_icon_shortcode($atts, $content = null){
		extract( shortcode_atts(array('type' => '', 'color'=>'', 'size'=>'', 'text_color'=>'',
			'link'=>'', 'target'=>'_self', 'padding_right'=>'', 'text_size'=>'13px', 'text'=>''), $atts) );	
		...
		$ret  = '';
		if( !empty($text) || !empty($padding_right) ){
			$ret .= '<span class="gdlr-icon-shortcode" >';
		}
		
		$ret .= (empty($link))? '': '<a href="' . $link . '" target="' . $target . '" >';
		$ret .= '<i class="limoking-icon fa ' . limoking_fa_class($type) . '" ' . $style . ' ></i>';
		$ret .= (empty($link))? '': '</a>';
		
		if( !empty($text) || !empty($padding_right) ){
			$ret .= "<span class=\"limoking-icon-inline-text\" style=\"font-weight: bold; text-transform: uppercase; margin-left: 6px; ";
			$ret .= empty($text_color)? '': "color: {$text_color}; ";
			$ret .= "padding-right: {$padding_right}; font-size: {$text_size}; \" >{$text}</span>";
			$ret .= '</span>';
		}
		return $ret;
	}		
	...
	function gdlr_notification_shortcode( $atts, $content = null ){
		...
		if( function_exists('limoking_get_notification_item') ){
			return '<div class="limoking-shortcode-wrapper">' . limoking_get_notification_item($settings) . '</div>';
		}		
		return '';
	}	
	...
	function gdlr_box_icon_shortcode( $atts, $content = null ){
		...
		if( function_exists('limoking_get_box_icon_item') ){
			return '<div class="limoking-shortcode-wrapper">' . limoking_get_box_icon_item($settings) . '</div>';
		}
		return '';		
	}
	...
	function gdlr_styled_box_shortcode( $atts, $content = null ){
		...
		if( function_exists('limoking_get_styled_box_item') ){
			return '<div class="limoking-shortcode-wrapper">' . limoking_get_styled_box_item($settings) . '</div>';
		}
		return '';
	}
	...
	function gdlr_testimonial_shortcode( $atts, $content = null ){
		...
		if( function_exists('limoking_get_testimonial_item') ){
			return '<div class="limoking-shortcode-wrapper">' . limoking_get_testimonial_item($settings) . '</div>';
		}
		return '';
	}
	...
	function gdlr_personnel_shortcode( $atts, $content = null ){
		...
		if( function_exists('limoking_get_personnel_item') ){
			return '<div class="limoking-shortcode-wrapper">' . limoking_get_personnel_item($settings) . '</div>';
		}
		return '';
	}	
	...
	function gdlr_blog_shortcode( $atts, $content = null ){
		...
		if( function_exists('limoking_get_blog_item') ){
			return '<div class="limoking-shortcode-wrapper">' . limoking_get_blog_item($settings) . '</div>';
		}
		return '';
	}		
	...
	function gdlr_portfolio_shortcode( $atts, $content = null ){
		...
		if( function_exists('limoking_print_portfolio_item') ){
			return '<div class="limoking-shortcode-wrapper">' . limoking_print_portfolio_item($settings) . '</div>';
		}
		return '';
	}	
	...
	function gdlr_page_shortcode( $atts, $content = null ){
		...
		if( function_exists('limoking_get_page_list_item') ){
			return '<div class="limoking-shortcode-wrapper">' . limoking_get_page_list_item($settings) . '</div>';
		}
		return '';
	}	
	...
	function gdlr_post_slider_shortcode( $atts, $content = null ){
		...
		if( function_exists('limoking_get_post_slider_item') ){
			return '<div class="limoking-shortcode-wrapper">' . limoking_get_post_slider_item($settings) . '</div>';
		}
		return '';
	}	
	...
	function gdlr_code_shortcode($atts, $content = null){
		...
		$ret  = '<div class="limoking-code-item ' . $active . '">';
		$ret .= '<div class="limoking-code-title"><i class="icon-' . $icon_class . '"></i>' . $title . '</div>';
		$ret .= '<div class="limoking-code-content">' . $content . '</div>';
		$ret .= '</div>';
	...
	function gdlr_gallery_shortcode($atts){
		if( !empty($atts['masterslider']) && $atts['masterslider'] == "true" ){ return gallery_shortcode($atts); }
	
		extract( shortcode_atts(array('columns' => '3', 'ids'=>'', 'link'=>'post', 'type'=>'grid',
			'caption' => 'yes', 'caption_position'=>'left', 'thumbnail_size'=>'thumbnail', 'size'=>''), $atts) );	
		...
		foreach( $ids as $slide_id ){
			...
			if($type == 'slider' && $caption == 'yes'){
				$attachment = limoking_get_attachment_info($slide_id);
		...
		if( function_exists('limoking_get_flex_slider') && $type == 'slider' ){
			$settings['slider-type'] = 'flexslider';
			return limoking_get_flex_slider($slides, array('size'=> $thumbnail_size));
		}else if( function_exists('limoking_get_gallery_thumbnail') && $type == 'thumbnail' ){
			$settings['slider'] = $slides;
			return '<div class="limoking-shortcode-wrapper">' . limoking_get_gallery_thumbnail($settings) . '</div>';
		}else if( function_exists('limoking_get_gallery') ){
			$settings['slider'] = $slides;
			return '<div class="limoking-shortcode-wrapper">' . limoking_get_gallery($settings) . '</div>';
		}
		return '';		
	}
	...
	function gdlr_title_shortcode( $atts, $content = null ){
		...
		return '<div class="limoking-shortcode-wrapper">' . limoking_get_item_title($settings) . '</div>';
	}	
