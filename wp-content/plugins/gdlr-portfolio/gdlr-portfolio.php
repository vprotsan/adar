...
add_action('plugins_loaded', 'limoking_portfolio_init');
if( !function_exists('limoking_portfolio_init') ){
	function limoking_portfolio_init() {
...
add_action( 'init', 'limoking_create_portfolio' );
if( !function_exists('limoking_create_portfolio') ){
	function limoking_create_portfolio() {
		global $theme_option;
		...
		if( defined('WP_THEME_KEY') && WP_THEME_KEY == 'goodlayers' ){
			if( !empty($theme_option['portfolio-page-style']) && $theme_option['portfolio-page-style'] == 'blog-style' ){
				add_filter('single_template', 'limoking_register_portfolio_blog_template');
			}else{
				add_filter('single_template', 'limoking_register_portfolio_template');
			}
		}
	}
}

if( !function_exists('limoking_register_portfolio_template') ){
	function limoking_register_portfolio_template($single_template) {
		global $post;
...
if( !function_exists('limoking_register_portfolio_blog_template') ){
	function limoking_register_portfolio_blog_template($single_template) {
		global $post;
...
add_filter('get_previous_post_where', 'limoking_portfolio_post_where', 10, 2);
add_filter('get_next_post_where', 'limoking_portfolio_post_where', 10, 2);
if(!function_exists('limoking_portfolio_post_where')){
	function limoking_portfolio_post_where( $where, $in_same_cat ){ 
		global $post; 
		if ( $post->post_type == 'portfolio' ){
